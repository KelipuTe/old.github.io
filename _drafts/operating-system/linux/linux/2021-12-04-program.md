...
c 源码文件通过 gcc 处理会得到 elf 可执行文件。gcc 处理包括：

- preprocessing（预处理），预处理会增加一些代码。（预处理器）
- compilation（编译），编译得到汇编代码。（编译器）
- assemble（汇编），汇编把汇编代码转化为机器指令。（汇编器）
- link（链接），链接把各个模块连接起来组织成为可执行文件。（连接器）

在链接中，把函数的名字和变量的名字都称为 symbol（符号）。源代码中的函数的名字和变量的名字，无论长度多少，最终都会转换为符号。
...
三个命令的结果第一行都会输出 `file format elf64-x86-64v` ，这个表示文件类型。
...
- `Contents of section .text`，每行从第二段开始，表示程序指令的内容（16 进制机器指令）。后面的 `UH............].` 是程序指令的 ASICC 文本。
...
从上面三组结果可以看出，代码段 `Disassembly of section .text` 的第一个函数不是 `main` 函数，而是 `_start` 函数。`_start` 函数会调用 `__libc_start_main` 函数进行一些必要的初始化操作，然后再调用 `main` 函数。